...
## **5. Vibe Infrastructure Protocol**

All blueprint-compliant apps MUST adhere to these environment and security standards:

*   **Registry Identity:** Every app MUST define a `VITE_VIBE_REPO_ID` in its `.env` to link with the Vibe OS Command Center.
*   **Multi-Stage .env:** Use the 4-stage (`local`, `development`, `testing`, `production`) commented block pattern defined in `.env.template`.
*   **The VibeGuard:** Before rendering core UI, the app MUST execute the `EnvironmentGuard` component.
    *   **Auth Requirement:** The guard must require a Firebase/Google login before revealing setup instructions.
    *   **Mode Awareness:** In `development`, show detailed missing keys + Deep Link to Vibe Admin. In `production`, show a simplified recovery view.
*   **Gemini Key Distinction:** Clearly separate `VITE_GEMINI_API_KEY` (used for AI/Agent tasks) from `APP_GEMINI_API_KEY` (used for runtime application features).
